...
static uint64_t *key_ptr(struct node *n, uint32_t index)
{
	return n->keys + index;
}
...
int btree_node_merge(struct shadow_spine *s, unsigned parent_index, size_t value_size)
{
	struct node *p = shadow_parent(s)->n, *c = shadow_current(s)->n;
	...
	if (is_just(&left_count) && (left_count.value < merge_threshold(c))) {
		...
		left.b = value64(p, parent_index - 1);
		...
		*key_ptr(p, parent_index) = *key_ptr(c, 0);
		...
		right.b = value64(p, parent_index + 1);
